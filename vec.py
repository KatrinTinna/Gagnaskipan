...
def add(u,v):
    ...
    # sum_vec.D = u.D
    for i in u.D:
        sum_vec.f[i] = u.f.get(i,0) + v.f.get(i,0)
        if i in u.f.keys():
            if i in v.f.keys():
                sum_vec.f[i] = u.f[i] + v.f[i]
            else:
                sum_vec.f[i] = u.f[i]
        elif i in v.f.keys():
                sum_vec.f[i] = v.f[i]
...
def scalar_mul(v, alpha):
    ...
    ret_dict = {}
    for key in v.D:
        ret_dict[key] = alpha * v.f.get(key, 0)

    return_vec = Vec(v.D, ret_dict)
    return return_vec
...
def neg(v):
    ...
    True
    """
    negation_vec = Vec(v.D, {})
    for i in v.f.keys():
        negation_vec.f[i] = -v.f[i]
    return negation_vec
