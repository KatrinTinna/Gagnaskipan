...
def equal(A, B):
    ...
    if A.D != B.D :
        return False
    #A.D[0] fyrsta túpla í D.
    for i in A.D[0]:
        #A.D[1] second túpla í D.
        for a in A.D[1]:
            if A.f.get((i,a),0) != B.f.get((i,a),0):
                return False
    else:
        return True
...
def add(A, B):
    ...
    for i in A.D[0]:
        for a in A.D[1]:
            result = A.f.get((i,a), 0) + B.f.get((i,a), 0)
            if result != 0:
                return_mat.f[i,a] = result
    return return_mat
...
def scalar_mul(M, x):
    ...
    for i in M.D[0]:
        for a in M.D[1]:
            result = M.f.get((i,a),0)*x
            if result !=0:
                new_mat.f[(i,a)] = result
    return new_mat
...
def transpose(M):
    ...
    True
    """
    M_transpose_labels = (M.D[1], M.D[0])
    M_transpose = Mat(M_transpose_labels, {})
    for i in M.D[0]:
        for a in M.D[1]:
            if M.f.get((i,a),0) != 0:
                M_transpose[(a,i)] = M.f.get((i,a),0)
    return M_transpose
...
def vector_matrix_mul(v, M):
    ...
    for i in M.D[0]:
        result = 0
        for a in M.D[1]:
            if ((i,a) in M.f) and (i in v.f):
                return_vec[a] += M.f[(i,a)]*v.f[i]
            else:
                return_vec[a] += 0
    return return_vec
